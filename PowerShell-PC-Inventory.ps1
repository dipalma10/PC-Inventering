...
New-Item -ItemType Directory "C:\windows\logs\Programs" -Force

Start-Transcript "C:\windows\logs\Programs\invent.log" 
...
$Speed = Get-WmiObject -Class win32_processor | Select-Object -ExpandProperty MaxClockSpeed

$cpuSpeed = ((get-wmiobject Win32_Processor).MaxClockSpeed)/1000
$cpuspeedstring = ("{0}GHz" -f  $cpuspeed)
$cpuspeedstring
...
function OutputToCSV {
  ...
  Add-Member -InputObject $infoObject -MemberType NoteProperty -Name "CPU Speed" -Value $cpuspeedstring
  ...
  Add-Member -InputObject $infoObject -MemberType NoteProperty -Name "GPU 2" -Value $GPU2
